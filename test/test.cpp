...
TEST(PIDControllerTest, TestClamping) {
    ...
    // Since the error is large and the gains are high, the expected unclamped output will be much greater than max.
    // We expect the output to be clamped to the maximum value of 50.0.
...
TEST(PIDControllerTest, TestCalculateOutput) {
    ...
    ASSERT_NEAR(output, expected_output, 0.01);  // Allowing for small floating-point errors
}
